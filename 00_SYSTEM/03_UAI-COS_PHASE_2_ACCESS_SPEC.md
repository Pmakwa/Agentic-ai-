<!--
UAI-COS PHASE-2 SPEC — ACCESS PATH, REACHABILITY & CAPABILITY EXPANSION BLUEPRINT (imported VERBATIM)
Source        : user message (Arena chat), 2026-09-23 — poora Phase 2 prompt user ne khud paste kiya
Imported on   : 2026-09-23 (Asia/Kolkata)
Extraction    : direct paste — HTML entities (&amp; -> &) normalize kiye, baaki ek shabd nahi badla
Content hash  : sha256:8a801fdcfc9329e7
Sections      : 32 numbered sections + CORE MISSION + INITIALIZATION COMMAND
Status        : CANONICAL — verbatim. Edit nahi karte; naya version = nayi file.
Apply engine  : python3 tools/apply_phase.py --spec phase2  (access-path exploration + attestation)
Raw copy      : 00_SYSTEM/_raw/prompt_phase_2_access_raw.md
Phase 1 spec  : 00_SYSTEM/02_UAI-COS_MASTER_SELF_AUDIT_SPEC.md (self-discovery blueprint)
-->

> **Import note:** Ye user ke original **Phase 2** prompt ka poora, uncut text hai — "access nahi hai" bolne ke bajaye
> **legitimate alternative routes** dhoondhne ka master blueprint. Isko canonical maan kar apply kiya gaya:
> deliverable `03_ACCESS_EXPANSION/01_PHASE2_BLUEPRINT_APPLY.md`, blocks `04_GITHUB_UNLOCK/`, `05_SOCIAL_UNLOCK/`, `06_REPO_HUNT/`.
