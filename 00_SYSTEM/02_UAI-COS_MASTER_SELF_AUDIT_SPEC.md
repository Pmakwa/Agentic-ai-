<!--
UAI-COS MASTER SELF-AUDIT SPEC — IMPORTED VERBATIM (ye user ka original Phase 1 + Phase 2 structured prompt hai)
Source        : user message (Arena chat), 2026-09-23 — poora prompt user ne khud paste kiya
Imported on   : 2026-09-23 (Asia/Kolkata)
Extraction    : direct paste (koi reader-proxy nahi) — HTML entities (&amp; -> &) normalize kiye, baaki ek shabd nahi badla
Content hash  : sha256:e8d8620d378adc48
Sections      : 35 numbered sections (1, 3-12, 14-26, 28-35) + ROLE + INITIALIZATION COMMAND (PHASE 1-15)
Covers        : Phase 1 (self-capability inventory / zero-assumption) + Phase 2 (access & research mapping, fallbacks)
Status        : CANONICAL — verbatim. Edit nahi karte; naya version = nayi file.
Apply engine  : python3 tools/apply_phase.py --spec self_audit  (har agent apne environment me apply karke attestation deta hai)
Raw copy      : 00_SYSTEM/_raw/prompt_master_self_audit_raw.md
-->

> **Import note:** Ye file user ke original structured prompt ka **poora, uncut** text hai (Phase 1 + Phase 2 ka source).
> Isko canonical maan kar implement kiya gaya: yahan `07_SELF_AUDIT/` deliverables bane, aur repo me apply-system juda.
