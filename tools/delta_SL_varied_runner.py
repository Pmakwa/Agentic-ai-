#!/usr/bin/env python3
"""
Delta India — SL 1% / 1.5% / 2% × RR 1:2 / 1:3 × 5m/15m/30m/1h × 3 coins = 72 tests
Capital ₹10,000 | Risk 1% fixed (₹100) | SL varied | Delta fees 0.059%
Replaces 8% SL which was weekly, now intraday-appropriate SL
"""
import sys
...
for coin in COINS:
    for tf in TIMEFRAMES:
        ...
        if len(df)<100:
            continue
        for sl_label, sl_val in SL_LEVELS.items():
            for rr_label, rr_val in RR_MAP.items():
                print(f"\n[RUN] {coin} {tf} SL {sl_label} RR {rr_label} Risk 1% ...", flush=True)
                res=backtest_double_st_intraday(df, coin, tf, 10,3.0,10,4.0, sl_val, rr_val, RISK_PCT, True, risk_fixed=None)
                m=res["metrics"]
                ...
                fname=f"TRADE_LOG_{coin.replace('/','_')}_{tf}_SL{sl_label.replace('%','pct')}_{rr_label.replace(':','-')}_1pctRisk.csv"
                pd.DataFrame(res["trades"]).to_csv(OUT / fname, index=False)
                with open(OUT / f"RESULT_{coin.replace('/','_')}_{tf}_SL{sl_label.replace('%','pct')}_{rr_label.replace(':','-')}_1pctRisk.json","w") as f:
                    json.dump({k:v for k,v in res.items() if k!="trades"}, f, indent=2, default=str)
                all_rows.append({
                    "coin":coin,"timeframe":tf,"bars":res["bars"],"sl":sl_label,"rr":rr_label,"risk":"1%",
                    "sl_pct":sl_val*100,"rr_val":rr_val,"strategy":"Double ST 10x3+10x4",
...
pd.DataFrame(all_rows).to_csv(OUT/"MASTER_SL1_1.5_2_RR12_13_72.csv", index=False)
with open(OUT/"SUMMARY_SL.json","w") as f: json.dump(all_rows,f,indent=2)
print(f"\n[DONE] {len(all_rows)} configs")
df=pd.DataFrame(all_rows)
for sl in ["1%","1.5%","2%"]:
    print(f"\n=== SL {sl} ===")
    sub=df[df["sl"]==sl]
    print(sub.groupby("rr")[["total_net_profit","profit_factor"]].mean())
    for rr in ["1:2","1:3"]:
        print(f" RR {rr}:")
